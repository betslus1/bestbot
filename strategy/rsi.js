...
module.exports.step = function (lastCandles, currentOrder, currentBalance) {
	...
	let indLog = [];
	for (let i of Object.keys(module.exports.indicators) ){
		indLog.push([i, indicators[i]]);
	}
	logs.push(`Индикаторы:`, indLog);

	logs.push({currentBalance});


	var isBuy = true;
	var isSell = true;
	for (let i of Object.keys(module.exports.indicators)){		
		if (module.exports.indicators[i]?.buy){
			if(indicators[i] > module.exports.indicators[i]?.buy){
				isBuy = false;
			}
			logs.push(`[BUY]\tПроверка индикатора ${i}: \t` + 
				`${indicators[i]} < ${module.exports.indicators[i]?.buy} \t ${indicators[i] > module.exports.indicators[i]?.buy?'Покупка запрещена':'Покупка разрешена'} `);
		}

		if (module.exports.indicators[i]?.sell){
			if (indicators[i] < module.exports.indicators[i]?.sell){
				isSell = false;
			}
			logs.push(`[SELL]\tПроверка индикатора ${i}: \t` + 
				`${indicators[i]} > ${module.exports.indicators[i]?.sell} \t ${indicators[i] < module.exports.indicators[i]?.sell?'Продажа запрещена':'Продажа разрешена'} `);
		}
	}

	logs.push('Текущие заявки:', 'Buy: ' + currentOrder.Buy?.initial_order_price??false, 'Sell: ' + currentOrder.Sell?.initial_order_price??false);
